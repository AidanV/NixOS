...
function StartSection() {
  const SideBarButton = Widget.Button({
    ...
    cursor: 'pointer',
    child: Widget.Label('A'), //Widget.Box({
      //css: `background-image: url("/home/${Utils.exec('whoami')}/.face.icon")`
    //}),
    onPrimaryClick: () => Utils.exec(`echo here; bash -c "${App.configDir}/shared/scripts/sidebar.sh toggle"`)
  })
...
//     spacing: 4,
//     child: Widget.Box({
//       children: Array.from({ length: 10 }).map((_, i) => i + 1).map(i => Widget.Button({
//       attribute: i,
//       label: `${i}`,
...
//       hpack: 'center',
//       cursor: 'pointer',
//       setup: self => self.hook(HyprlandService, () => self.children.forEach(btn => {
//         btn.visible = HyprlandService.workspaces.some(ws => ws.id === btn.attribute);
//       }))
//     }))
//   })})
// 
  ...
  const WorkspaceIndicator = Widget.Box({
    ...
    children: Array.from({ length: 10 }).map((_, i) =>
      Widget.Button({
        ...
        self.visible = i < HyprlandService.workspaces.length;
      })
    )
  })
